from shoes import Shoes,ColorInfo
import pymysql
import pickle
import datetime
...
class MysqlOperator:
    def __init__(self,ip_address='db', uname='root', password='123456', db_name='db_shoes'):
        self.db_connection = pymysql.connect(ip_address,uname,password,db_name)
    ...
    def search_by_name(self,name):
        ...
        result_dict = dict()
        try:
            result_dict['shoes_id'],result_dict['shoes_name'] = self.cursor.fetchone()
        except:
            return None
    ...
    def search_color_id(self,shoes_id):
        sql = "SELECT DISTINCT a.color_name,a.img_link \
                FROM ShoesColor a INNER JOIN ShoesPrice b \
                on a.color_id=b.shoes_color_id \
                WHERE shoes_id=%s"%shoes_id
        self.sql_excute(sql)  
        result_dict = dict() 
        result_dict['color_name'],result_dict['img_link'] = [],[]
        try:
            for row in self.cursor.fetchall():
                result_dict['color_name'].append(row[0])
                result_dict['img_link'].append(row[1])
        except:
            return None
    ...
    def search_price_data(self,shoes_id,color_id,size):
        ...
        sql = "SELECT price,update_time as date \
                FROM ShoesPrice \
                WHERE shoes_id=%s and shoes_color_id=%s and size=%s "%(shoes_id,color_id,self.size_table.index(str(size)))
        self.sql_excute(sql)  
